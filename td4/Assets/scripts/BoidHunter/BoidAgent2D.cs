...
public class BoidAgent2D : Agent
{
    ...
    public override void CollectObservations(VectorSensor sensor)
    {
        ...
        Vector2 hunterOffset = (Vector2)manager.HunterPosition - currentPosition;
        float hunterDistance = hunterOffset.magnitude;
        Vector2 flockCenterOffset = GetFlockCenter(currentPosition) - currentPosition;

        sensor.AddObservation(rb != null ? rb.linearVelocity : velocity);
        sensor.AddObservation(hunterDistance);
        sensor.AddObservation(flockCenterOffset);
    ...
    {
        if (manager == null || !manager.IsGameRunning || controlMode != BoidControlMode.MlAgent)
        {
            return;
        }

        float forceX = Mathf.Clamp(actions.ContinuousActions[0], -1f, 1f);
        float forceY = Mathf.Clamp(actions.ContinuousActions[1], -1f, 1f);
        float rotateInput = Mathf.Clamp(actions.ContinuousActions[2], -1f, 1f);

        ApplySteeringAndRewards(new Vector2(forceX, forceY) * maxSteerForce, rotateInput);
    }
